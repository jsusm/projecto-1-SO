...
int packupModifiedFiles(const char *logDirectory, struct List *modifiedList) {
    ...
    if (verifyDirectoryExistence() == -1) { //Verify if /var/log/PROYECTO SO 1/logs exists and is a Directory
        return 1;
    }

    //Asign a name for the .pak file
    strftime(timestamp_str, sizeof(timestamp_str), "%Y-%m-%d-%H-%M-%S", t);
    snprintf(pak_filepath, sizeof(pak_filepath), "/var/log/PROYECTO SO 1/logs/%s.pak", timestamp_str);
